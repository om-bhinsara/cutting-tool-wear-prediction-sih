...
// CNC Carbide Milling Tool Dataset Baseline Metrics (10 Tools to Failure, CK45 & RVS 304)
const TOOL_SPECS = {
  FAILURE_LIMIT_UM: 300.0,      // Critical failure threshold in µm
  NOMINAL_TOTAL_CYCLES: 315,    // Average cuts until failure (315 machining passes)
  BASE_WEAR_RATE: 300.0 / 315,  // ~0.9524 µm per machining pass
  STAGE_HEALTHY: 100.0,
  STAGE_MODERATE: 200.0,
  STAGE_HIGH: 300.0
};
...
export default function App() {
  ...
  const [progressionHistory, setProgressionHistory] = useState(() => {
    const saved = localStorage.getItem('cad_wear_progression_final_v2');
  ...
  useEffect(() => {
    localStorage.setItem('cad_wear_progression_final_v2', JSON.stringify(progressionHistory));
  }, [progressionHistory]);
  ...
  const clearLiveTracking = () => {
    if (window.confirm('Reset all live tracked machining wear cycles?')) {
      ...
      setMachiningCycle(1);
      localStorage.removeItem('cad_wear_progression_final_v2');
    }
  };
  ...
  // Real-Data Physics & Prognostics Calculations
  ...
  // Dynamic wear rate calculation from actual cutting passes
  ...
  if (progressionHistory.length >= 2) {
    ...
    if (deltaWear > 0) {
      dynamicWearRate = deltaWear / deltaCycles;
    }
  }

  // True Remaining Useful Life (RUL in machining passes)
  ...
  if (progressionHistory.length >= 2) {
    ...
    if (diff > 0.3) {
      wearTrendText = `↑ Increasing (+${diff.toFixed(2)} µm/cycle)`;
      wearTrendColor = 'text-danger';
    } else if (diff < -0.3) {
      wearTrendText = `↓ Decreasing (${diff.toFixed(2)} µm)`;
      wearTrendColor = 'text-success';
    } else {
      wearTrendText = '→ Steady State';
      wearTrendColor = 'text-primary';
    }
  } else if (result) {
    wearTrendText = '↑ Active Sensor Telemetry';
    wearTrendColor = 'text-primary';
  }
      ...
      <header className="sticky-top border-bottom" style={{ background: 'rgba(255, 255, 255, 0.95)', backdropFilter: 'blur(12px)', borderColor: '#cbd5e1', zIndex: 50 }}>
        <div className="container d-flex flex-wrap justify-content-between align-items-center py-2" style={{ maxWidth: '1280px' }}>
            ...
            <div>
              <div className="fw-bold text-dark fs-5 tracking-tight d-flex align-items-center gap-2">
                ...
                <span className="badge bg-primary bg-opacity-10 text-primary border border-primary border-opacity-25 font-mono" style={{ fontSize: '10px' }}>
                  CNC MILLING PROGNOSTICS
                </span>
              </div>
              <div className="text-secondary font-mono" style={{ fontSize: '11px' }}>
                10 CARBIDE TOOLS TO FAILURE (CK45 &amp; RVS 304) &bull; RUL MONITOR
              </div>
            </div>
          </div>
          ...
          <div className="d-flex gap-2">
            {[
              { id: 'predict', label: 'Live Wear & RUL', icon: 'bi-activity' },
              { id: 'specs', label: 'Tool & Machine Specs', icon: 'bi-tools' },
              { id: 'implementation', label: 'Architecture & CAM', icon: 'bi-diagram-3-fill' }
            ].map((tab) => (
              <button
                key={tab.id}
                onClick={() => setActiveTab(tab.id)}
                className={`btn btn-sm px-3 py-2 fw-semibold d-flex align-items-center gap-2 rounded-2 ${activeTab === tab.id
          ...
          <div>
            ...
            <div className="cad-panel mb-4 p-4 text-center">
              ...
              <h2 className="fw-bold text-dark mb-1">
                CNC Carbide Tool Health &amp; Digital Wear Curve
              </h2>
              <p className="text-secondary font-mono small mb-0">
                Calibrated across 10 CNC Carbide Cutters (CK45 &amp; RVS 304 Steel) &bull; Standard Failure Limit: <strong>300 µm (315 Cycles)</strong>
              </p>
            </div>

            {/* ============= 1. TOOL HEALTH TRACKER (BOUNDS-SAFE 0 TO 300 µm SCALE) ============= */}
              ...
              <div className="col-12">
                <div className="cad-panel p-4" style={{ background: '#0b111e', color: '#f8fafc', borderColor: '#1e293b' }}>
                  ...
                  <div className="d-flex flex-wrap justify-content-between align-items-center mb-4 pb-2 border-bottom border-secondary border-opacity-25">
                    ...
                    <div className={`badge bg-${currentWear > 300 ? 'danger' : currentWear > 200 ? 'warning text-dark' : 'success'} font-mono px-3 py-2 fs-6 shadow`}>
                      <i className="bi bi-shield-check me-1"></i> Recommendation: {
                        currentWear <= 100 ? 'Optimal Condition — Continue Normal Operation' :
                          currentWear <= 200 ? 'Active Steady Wear — Continue Monitoring' :
                            currentWear <= 300 ? 'High Flank Wear Detected — Prepare Tool Replacement' :
                              'Critical Failure Limit Exceeded — Replace Tool Immediately'
                      }
                    </div>
                  </div>

                  {/* Digital Linear Wear Gauge */}
                  <div className="py-4 px-3">
                    <div className="position-relative w-100" style={{ height: '95px' }}>

                      {/* Underlying Baseline Rail */}
                      <div
                      ...
                      {/* Fixed Benchmark Ticks with Edge-Bound Alignment */}
                      {[
                          ...
                          >
                            <span
                              ...
                              style={{
                                ...
                                color: tick.color,
                                whiteSpace: 'nowrap',
                                transform: tick.align === 'end' ? 'translateX(-30%)' : tick.align === 'start' ? 'translateX(25%)' : 'none'
                              }}
                            >
                              {tick.label}
                            </span>
                      ...
                      {/* Live Current Tool Pointer Marker */}
                      {currentWear > 0 && (
                        <div
                        ...
                        >
                          <div
                            style={{
                              width: '16px',
                              height: '16px',
                              backgroundColor: currentWear > 300 ? '#ef4444' : currentWear > 200 ? '#f97316' : currentWear > 100 ? '#f59e0b' : '#00f2fe',
                          ...
                          <div className="d-flex flex-column align-items-center mt-1">
                            ...
                            <div
                              className="badge bg-light text-dark font-mono px-2 py-1 shadow fw-bold mt-1"
                              style={{ fontSize: '12px', border: '1px solid #00f2fe', whiteSpace: 'nowrap' }}
                            >
                    ...
                    <div className="row g-3 pt-3 font-mono small border-top border-secondary border-opacity-25 mt-3">
                      ...
                      <div className="col-md-3">
                        <span className="text-secondary d-block">Est. Remaining Useful Life (RUL):</span>
                        <span className={`fw-bold ${estimatedRemainingCycles <= 20 ? 'text-danger' : estimatedRemainingCycles <= 80 ? 'text-warning' : 'text-success'}`}>
                          ~{estimatedRemainingCycles} Machining Cycles ({((estimatedRemainingCycles / TOOL_SPECS.NOMINAL_TOTAL_CYCLES) * 100).toFixed(0)}% passes)
                        </span>
                      </div>

                      <div className="col-md-3">
                        <span className="text-secondary d-block">Health Stage Classification:</span>
                        <span className={`badge bg-${currentWear <= 100 ? 'success' : currentWear <= 200 ? 'warning text-dark' : currentWear <= 300 ? 'danger' : 'dark'} font-mono px-2 py-1`}>
                ...
                <div className="cad-panel h-100">
                  ...
                  <div className="p-4">
                    <form onSubmit={handlePredict}>
                      ...
                      <div className="mb-4">
                        <label className="form-label text-dark fw-semibold small d-flex justify-content-between mb-1">
                          <span>3. Machining Cycle Index (1 – 315)</span>
              ...
              <div className="col-lg-7">
                <div className="cad-panel h-100 d-flex flex-column justify-content-between">
                  ...
                  <div className="p-3 bg-light border-top font-mono small text-secondary d-flex justify-content-between align-items-center">
                    <span><i className="bi bi-shield-check text-primary me-2"></i>Grad-CAM Layer: <code>model.image.net[6]</code> (Conv2d 64&rarr;96)</span>
            ...
            {/* ============= LIVE GRAD-CAM (ORDER: 1. Heatmap, 2. Overlay, 3. Original) ============= */}
            {result?.gradcam && (
              <motion.div
                    ...
                    <div className="d-flex justify-content-between align-items-center mb-3 pb-2 border-bottom">
                      <div>
                        <h5 className="fw-bold text-dark mb-1 font-mono d-flex align-items-center gap-2">
                          <i className="bi bi-eye-fill text-primary"></i> MATWI Grad-CAM Visual Explainability (Layer 6: Conv2d)
                        </h5>
                        <p className="text-secondary font-mono small mb-0">
                          Neural gradient activation map ($\gamma = 0.8$, $\alpha = 0.35$) showing regions dictating tool flank wear prediction.
                        </p>
                      </div>
                    ...
                    <div className="row g-4 text-center">
                      ...
                      <div className="col-md-4">
                        <div className="p-3 bg-light border rounded h-100 d-flex flex-column justify-content-between">
                          <span className="font-mono small fw-bold text-primary d-block mb-2">
                            1. Grad-CAM Heatmap
                          </span>
                      ...
                      <div className="col-md-4">
                        <div className="p-3 bg-light border rounded h-100 d-flex flex-column justify-content-between">
                          <span className="font-mono small fw-bold text-danger d-block mb-2">
                            2. Heatmap Overlay ($\alpha = 0.35$)
                          </span>
                          <div className="my-auto">
                            <img
                          ...
                          <span className="font-mono text-danger small mt-2" style={{ fontSize: '11px' }}>Tool Edge Attention Superimposed</span>
                        </div>
                      </div>
                      ...
                      <div className="col-md-4">
                        <div className="p-3 bg-light border rounded h-100 d-flex flex-column justify-content-between">
                          <span className="font-mono small fw-bold text-secondary d-block mb-2">
                            3. Original Optical Micrograph
                          </span>
                          <div className="my-auto">
                            <img
                              src={result.gradcam.original}
                              alt="Original Optical Micrograph"
                              className="img-fluid rounded border shadow-sm"
                              style={{ maxHeight: '220px', width: '100%', objectFit: 'contain' }}
                            />
                          </div>
                          <span className="font-mono text-muted small mt-2" style={{ fontSize: '11px' }}>Raw 224×224 Flank Face</span>
                        </div>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="d-flex flex-wrap justify-content-between align-items-center mb-3 pb-2 border-bottom">
                    <div>
                      <h5 className="fw-bold text-primary mb-1 font-mono d-flex align-items-center gap-2">
                        <i className="bi bi-graph-up-arrow"></i> Step 3 — Measurement $\rightarrow$ Wear $\rightarrow$ Time Trajectory
                      </h5>
                      <p className="text-secondary font-mono small mb-0">
                        {progressionHistory.length > 0
                          ? `Plotting ${progressionHistory.length} real wear point(s) tracked across CNC milling passes.`
                      ...
                      <ResponsiveContainer>
                        <LineChart data={progressionHistory} margin={{ top: 20, right: 30, left: 15, bottom: 20 }}>
                          ...
                          <XAxis
                            ...
                            label={{ value: 'Machining cycles (Passes) →', position: 'insideBottom', offset: -10, fill: '#1e293b', fontSize: 12, fontFamily: 'JetBrains Mono', fontWeight: 600 }}
                          />
                          <YAxis
                            stroke="#64748b"
                            tick={{ fill: '#475569', fontSize: 11, fontFamily: 'JetBrains Mono' }}
                          ...
                          <Tooltip
                            ...
                            labelFormatter={(label) => `Machining Cycle: Pass #${label}`}
                          />
                          <ReferenceLine
                            y={0.3}
                            label={{ value: 'CRITICAL FAILURE LIMIT (0.30 mm / 300 µm)', fill: '#dc2626', fontSize: 10, fontFamily: 'JetBrains Mono', fontWeight: 600, position: 'top' }}
                            stroke="#dc2626"
                            strokeWidth={1.5}
                            strokeDasharray="4 4"
                          />
                      ...
                      <div className="d-flex flex-wrap gap-2">
                        {CHANNEL_CONFIG.map((ch) => (
                          <button
                            ...
                            className={`btn btn-sm font-mono ${activeChannels.includes(ch.key)
            ...
            <div className="row g-4">
              <div className="col-md-6">
                <div className="cad-panel p-4 h-100">
                  ...
                  <h4 className="fw-bold text-dark mb-4 mt-1">CNC Milling Machine Setup</h4>

                  <div className="font-mono small">
                    <div className="d-flex justify-content-between py-2 border-bottom">
                      <span className="text-secondary">Machine Type</span>
                      <span className="text-dark fw-bold">CNC Milling Machine (Roders RFM760)</span>
                    </div>
                    <div className="d-flex justify-content-between py-2 border-bottom">
                      <span className="text-secondary">Tool Type</span>
                      <span className="text-dark fw-bold">Carbide Milling Tool / Carbide Cutter</span>
                    </div>
                    <div className="d-flex justify-content-between py-2 border-bottom">
                      <span className="text-secondary">Dataset Scope</span>
                      <span className="text-dark fw-bold">10 CNC Carbide Milling Tools to Failure</span>
                    </div>
                    <div className="d-flex justify-content-between py-2 border-bottom">
                      <span className="text-secondary">Force Channels</span>
                      <span className="text-dark fw-bold">Kistler Dynamometer ($F_x, F_y, F_z$)</span>
                    </div>
                    <div className="d-flex justify-content-between py-2 border-bottom">
                      <span className="text-secondary">Vibration Sensor</span>
                      <span className="text-dark fw-bold">3-Axis Accelerometer Signals</span>
                    </div>
                    <div className="d-flex justify-content-between py-2">
                      <span className="text-secondary">Acoustic Emission</span>
                      <span className="text-dark fw-bold">Kistler AE Sensor (High-Frequency)</span>
                    </div>
                  </div>
                </div>
              </div>
              ...
              <div className="col-md-6">
                <div className="cad-panel p-4 h-100">
                  ...
                  <span className="font-mono text-primary small fw-semibold">// METALLURGY &amp; WORKPIECE</span>
                  <h4 className="fw-bold text-dark mb-4 mt-1">Workpiece Materials &amp; Wear Criteria</h4>

                  <div className="font-mono small">
                    <div className="d-flex justify-content-between py-2 border-bottom">
                      <span className="text-secondary">Workpiece Material 01</span>
                      <span className="text-dark fw-bold">CK45 Carbon Steel</span>
                    </div>
                    <div className="d-flex justify-content-between py-2 border-bottom">
                      <span className="text-secondary">Workpiece Material 02</span>
                      <span className="text-dark fw-bold">RVS 304 Austenitic Stainless Steel</span>
                    </div>
                    <div className="d-flex justify-content-between py-2 border-bottom">
                      <span className="text-secondary">Primary Wear Mode</span>
                      <span className="text-dark fw-bold">Flank Wear ($V_B$) Growth</span>
                    </div>
                    <div className="d-flex justify-content-between py-2 border-bottom">
                      <span className="text-secondary">Nominal Tool Life</span>
                      <span className="text-dark fw-bold">~315 Machining Passes per Tool</span>
                    </div>
                    <div className="d-flex justify-content-between py-2">
                      <span className="text-secondary">Critical Failure Threshold</span>
                      <span className="text-danger fw-bold">$V_B \ge 300.0\ \mu\text{m}$ (0.30 mm)</span>
                    </div>
                  </div>
                </div>
              </div>
            ...
            <div className="row g-4">
              <div className="col-12">
                <div className="cad-panel p-4">
                  ...
                  <span className="font-mono text-primary small fw-semibold">// ARCHITECTURAL SPECIFICATION</span>
                  <h4 className="fw-bold text-dark mb-4 mt-1">Dual-Stream Multimodal PyTorch Pipeline</h4>
                    ...
                    <table className="table table-bordered font-mono small align-middle mb-0">
                      ...
                      <tbody>
                        ...
                        <tr>
                          ...
                          <td>5×512 Telemetry ($F_x, F_y, F_z$, Accel, AE)</td>
                        ...
                        <tr>
                          ...
                          <td>1d ($V_B$ in $\mu\text{m}$)</td>
                          <td>Flank wear regression ($R^2 = 0.9938$)</td>
                        </tr>
                      </tbody>
                    </table>
                  </div>
